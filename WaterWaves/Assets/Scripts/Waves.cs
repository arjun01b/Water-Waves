...
public class Waves : MonoBehaviour
{
    ...
    void Start()
    {
        //Mesh Setup
        Mesh = new Mesh();
        Mesh.name = gameObject.name; //same as object name

        Mesh.vertices = GenerateVerts(); //Specifying vertices
        Mesh.triangles = GenerateTries(); //Specifying triangles
        Mesh.uv = GenerateUVs();
        Mesh.RecalculateNormals();
        Mesh.RecalculateBounds(); //to ensure bounding volumes are correct

        MeshFilter = gameObject.AddComponent<MeshFilter>(); //Creating the mesh filter to actually render it
        MeshFilter.mesh = Mesh; //Returns the instantiated Mesh assigned to the mesh filter
    }
   ...
   public float GetHeight(Vector3 position)
   {
    ...
    private Vector3[] GenerateVerts()
    {
        ...
        for (int x = 0; x <= Dimension; x++)
            for (int z = 0; z <= Dimension; z++)
                verts[index(x, z)] = new Vector3(x, 0, z); //computing the index
        //
        return verts;
    }
    ...
    {
        var tries = new int[Mesh.vertices.Length * 6]; // 6 because a square is made of 2 traingles
        ...
        for (int x = 0; x < Dimension; x++)
        {
            for (int z = 0; z < Dimension; z++)
            {
                tries[index(x, z) * 6 + 0] = index(x, z); //Each index is multiplied by 6 based on the index number numbers are added
       ...
       for (int x = 0; x <= Dimension; x++)
       {
           for (int z = 0; z <= Dimension; z++)
           {
               ...
               for (int o = 0; o < Octaves.Length; o++)
               {
                   ...
                   {
                       var perl = Mathf.PerlinNoise((x * Octaves[o].scale.x) / Dimension, (z * Octaves[o].scale.y) / Dimension) * Mathf.PI * 2f; //introducing an element of pseudo-randomness
